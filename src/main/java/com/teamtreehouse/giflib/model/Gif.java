...
public class Gif {
    ...
    // store gif as file in a database (implemented by DBMS provider)
    // Lob - LargeOBjects
    @Lob
    private byte[] bytes;
    @NotNull
    @Size(min=3, max=12, message="Description must be between {min} and {max} characters long")
    private String description;

    // many gif objects can be associated with one category
    // gif field in Category.java is annotated @OneToMany
    @ManyToOne
    @NotNull
